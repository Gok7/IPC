...

{
  ...
  pid = getpid(); //récupére ID du process actif
  cle = atoi(argv[1]);  //copie la valeur entré dans cmd
  tps_travail = atoi(argv[2]); //copie la valeur saisi dans cmd pour temps de travail
  semid = semget(cle,1,0); //obtien ID de l'ensemble de sémaphores
